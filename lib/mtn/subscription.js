...
module.exports = {
	...
	async sendSmsMT(data) {
		return new Promise((resolve, reject) => {
			...
			const spInfo = { spId: data.spId, spPassword: data.spPwd }
	...
	startSmsMo(data) {
		return new Promise((resolve, reject) => {
			const param = {
				notify_url: data.notifyUrl, service: data.serviceId, shortcode: data.shortcode, correlator: data.correlatorId,
			}
			const spInfo = { spId: data.spId, spPwd: data.spPwd }

			mtn_sdp.startSmsNotification(param, spInfo, async (err, resp) => {
				if (err) {
					TerraLogger.debug(err, 'Error from MTN engine!')
					reject(err)
				}
