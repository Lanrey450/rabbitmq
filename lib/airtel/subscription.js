...
module.exports = {
	...
	async pushAirtelFeedbackToQueue(message) {
		try {
			await publish(config.rabbit_mq.airtel.postback_queue, message)
				.then((status) => {
					TerraLogger.debug(`pushed feedback data to queue: ${config.feedbackQueues.BillingFeedbackQUEUE}`)
				})
		} catch (err) {
