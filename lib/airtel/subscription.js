...
module.exports = {
			...
			return Axios({
				...
				if (response && response.data) {
					...
					case '5021': {
						...
						return publish(config.rabbit_mq.airtel.subscription_queue, {...subscriptionData.data, network: 'Airtel'})
							.then((status) => {
								TerraLogger.debug(`successfully pushed to the Airtel subscription data queue: ${status}`)
								return resolve({
									error: false,
			...
			return Axios({
				...
				responseJsonData = responseJsonData.Envelope.Body.handleDeSubscriptionResponse.handleDeSubscriptionReturn
					...
					case '1001': {
						...
						return publish(config.rabbit_mq.airtel.un_subscription_queue, {...unsubscriptionData.data, network: 'Airtel'})
							.then((status) => {
								TerraLogger.debug(`successfully pushed to the Airtel SE Requests data queue : ${status}`)
								return resolve({
									error: false,
	...
	async pushAirtelFeedbackToQueue(message) {
		try {
			await publish(config.airtel_feeback_queue, {...message.data, network: 'Airtel'})
				.then((status) => {
					TerraLogger.debug(`pushed feedback data to queue: ${config.airtel_feeback_queue}`)
				})
		} catch (err) {
			TerraLogger.debug(`error from pushing feedback data to queue: ${err}`)
		}
	},
