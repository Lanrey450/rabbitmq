...
module.exports = {
  ...
  sendSubscriptionRequest(msisdn, channel, service, route) {
    ...
    return new Promise((resolve, reject) => {
      ...
      return Axios({
        ...
        console.log(`Subscription SOAP Response Body for ${xmlRequestPayload} === ${responseString}`);
           ...
           switch (responseJsonData.errorCode) {
             case '1000': {
               ...
               return Utils.fetchServiceDetails(responseJsonData.productId)
                 .then((serviceDetails) => {
        ...
        console.log(`Could not push Airtel subscription to queue to be saved later in MongoDB: ${error}`);
        console.log(`Data not pushed : ${JSON.stringify(subscriptionData)}`);
        return resolve({
          error: false,
          message: `Subscription is successful for ${service.name}`,
          response: subscriptionData,
  ...
  /**
     ...
     * @returns {Promise}
     */
  putUserSubscription(subscriptionData) {
