...
module.exports = {
			...
			return Axios({
				...
				if (response && response.data) {
					...
					case '5021': {
						...
						return publish(config.rabbit_mq.airtel.subscription_queue, {...subscriptionData.data })
							.then((status) => {
			...
			return Axios({
				...
				responseJsonData = responseJsonData.Envelope.Body.handleDeSubscriptionResponse.handleDeSubscriptionReturn
					...
					case '1001': {
						...
						return publish(config.rabbit_mq.airtel.un_subscription_queue, {...unsubscriptionData.data })
							.then((status) => {
	...
	async pushAirtelPostbackToQueue(message) {
		try {
			await publish(config.rabbit_mq.airtel.postback_queue, {...message.data })
				.then(() => {
