...
module.exports = {
	...
	sendUserConsentSMS(params) {
		let { msisdn, shortCode, name, amount, validity } = params;
		let payload = {
			to: msisdn,
			network: '9Mobile',
			sender_id: shortCode,
	...
	sendUserSuccessMessageForUSSDSub(params) {
		let { msisdn, shortCode, name, amount, validity, unsubKey } = params;
		let payload = {
			to: msisdn,
			network: '9Mobile',
			sender_id: shortCode,
			content: `Hello, your subscription for the ${name} service was successful. Service costs N${amount}/${validity} days. To unsubscribe, text ${unsubKey} to ${shortCode}.`,
		}
	...
	sendUserSmsSub(data) {
		let { msisdn, shortCode, name, amount, validity, consent } = data;
		...
		let payload = {
			...
			network: '9Mobile',
			sender_id: shortCode,
			content: `Hello, your ${partMessage} for the ${name} Service was successful. To unsubscribe, text STOP to ${shortCode}. Service costs N${amount} / ${validity} days.`,
		}
	...
	sendUserOneOffSMS(data) {
		let { msisdn, shortCode, name, amount, validity } = data;

		let payload = {
			to: msisdn,
			network: '9Mobile',
			sender_id: shortCode,
			content: `Hello, your opt-in for the ${name} service was successful. To unsubscribe, text STOP to ${shortCode}. Service costs ${amount} / ${validity} days.`,
		}
	...
	sendUserBillingSMS(params) {
		...
		let payload = {
			...
			network: '9Mobile',
			sender_id: shortCode,
			content: `Hello, you have been charged N${amount} for ${name} service for ${validity} days. To unsubscribe, text ${keyword} to ${shortCode}.`,
		}
	...
	sendUserUnsubSMS(params) {
		let { msisdn, name, shortCode, keyword, channel } = params;
	...
	sendUserAutoRenewal(data) {
		...
		let payload = {
			...
			network: '9Mobile',
			sender_id: shortCode,
			content: `Hello, your renewal for the ${serviceName} Service was successful. To unsubscribe, text ${unSubscriptionKeyword} to ${shortCode}. Service costs N${amount} / ${validity} days.`,
		}
